import {Bodies, Common, Composite, Composites, Engine, Events, Render, Runner, World} from 'matter-js';
...
let body = Bodies.rectangle(width / 2, height / 2, 30, 30, {
  ...
  render: {strokeStyle: '#ffffff', sprite: {texture: '/warbird.png'}}
});
...
let arenaTop = Bodies.rectangle(width / 2, 100, width - 200, 30, {
  ...
  restitution: 1,
  collisionFilter: {category: 1},
  render: {fillStyle: my_gradient, strokeStyle: '#ffffff'}
});
...
let game;
let bodyAngle = 0, bodyAngleDeg = 0, speed = 0, velocityX = 0, velocityY = 0,
    forceX = 0, forceY = 0, accelX = 0, accelY = 0, bulletVelocityX = 0,
    bulletVelocityY = 0, lastShot = 0;
export default {
  data() {
    return {game: null};
  },
  async mounted() {
    ...
    // 'init' parses all available CSS and attach ResizeSensor to those elements
    // which have rules attached (make sure this is called after 'load' event,
    // because CSS files are not ready when domReady is fired.
    ...
    let ships = {
      0: {
        ...
        energy: {current: 1500, initial: 1500, recharge: 400, afterburner: 550},
        speed: {current: 0, initial: 200, max: 600, rotation: 200},
        thrust: {initial: 16, max: 24},
        maxSpeed: 6000,
        velocity: 200,
        speed: 2000
      }
    ...
    Matter.Events.on(game.runner, 'beforeTick', evt => {
      ...
      ctx.fillText(
          `bullet velocity:  x: ${bulletVelocityX}  y: ${bulletVelocityY}`, 5,
          100);
  ...
  methods:
      {
        shoot(game) {
          ...
          let bullet =
              Matter.Bodies.rectangle(body.position.x, body.position.y, 5, 5, {
                friction: 0,
                frictionStatic: 0,
                frictionAir: 0,
                restitution: 1,
                collisionFilter: {category: 4, mask: 1, group: -1},
                render: {strokeStyle: '#ffffff', fillStyle: '#ffffff'}
              });
          Matter.Body.setVelocity(
              bullet, Matter.Vector.sub(body.velocity, bulletVector));
        ...
        thrust(direction, boost) {
          ...
          forceVector =
              Matter.Vector.mult(forceVector, direction * boost * 0.0002);
